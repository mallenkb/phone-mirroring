...
final class NotificationLaunchTests: XCTestCase {
    ...
    // A calendar reminder's text is just the event time, which the shade's own
    // clock header matches exactly; the wordy title row must win instead.
    func testTapPointPrefersTitleRowOverHeaderClockForTimeOnlyText() {
        let lines = [
            ...
            NotificationTapService.ShadeTextLine(text: "PhoneRelay deep link check", center: CGPoint(x: 540, y: 1500)),
            NotificationTapService.ShadeTextLine(text: "01:32", center: CGPoint(x: 300, y: 1560))
        ]

        let point = NotificationTapService.forwardedNotificationTapPoint(
            in: lines,
            title: "PhoneRelay deep link check",
            text: "01:32"
        )
