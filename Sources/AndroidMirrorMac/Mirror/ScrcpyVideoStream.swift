...
/// sockets and parses the wire protocol into video packets + control
/// connection.
///
/// Wire format (reverse tunnel mode, video=true control=true audio=false):
///   socket 1 (video):
...
final class ScrcpyVideoStream {
    ...
    private func readUInt32BE(at offset: Int) -> UInt32 {
        let start = videoBuffer.index(videoBuffer.startIndex, offsetBy: offset)
        let bytes = videoBuffer[start..<videoBuffer.index(start, offsetBy: 4)]
        return bytes.reduce(0) { ($0 << 8) | UInt32($1) }
    }

    private func readUInt64BE(at offset: Int) -> UInt64 {
        let start = videoBuffer.index(videoBuffer.startIndex, offsetBy: offset)
        let bytes = videoBuffer[start..<videoBuffer.index(start, offsetBy: 8)]
