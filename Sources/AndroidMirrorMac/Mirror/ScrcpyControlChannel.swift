...
final class ScrcpyControlChannel {
    ...
    func sendScroll(normalized: CGPoint, deltaX: CGFloat, deltaY: CGFloat) {
        ...
        appendInt32BE(&buf, UInt32(bitPattern: x))
        appendInt32BE(&buf, UInt32(bitPattern: y))
        appendUInt16BE(&buf, deviceWidth)
        appendUInt16BE(&buf, deviceHeight)
        appendUInt16BE(&buf, UInt16(bitPattern: hFixed))
        appendUInt16BE(&buf, UInt16(bitPattern: vFixed))
        appendUInt32BE(&buf, lastButtons)
        write(buf)
    }
    ...
    private func sendKeycode(action: KeyAction, keycode: Int32, metastate: UInt32) {
        ...
        appendUInt32BE(&buf, UInt32(bitPattern: keycode))
        appendUInt32BE(&buf, 1) // repeat
        appendUInt32BE(&buf, metastate)
        write(buf)
    }
    ...
    private func send(touchAction: TouchAction, x: Int32, y: Int32, pressure: Float,
        ...
        appendUInt64BE(&buf, Self.pointerIDMouse)
        appendInt32BE(&buf, UInt32(bitPattern: x))
        appendInt32BE(&buf, UInt32(bitPattern: y))
        appendUInt16BE(&buf, deviceWidth)
        appendUInt16BE(&buf, deviceHeight)
        let pressureFP = UInt16(clamping: Int(min(1, max(0, pressure)) * 65535))
        appendUInt16BE(&buf, pressureFP)
        appendUInt32BE(&buf, actionButton)
        appendUInt32BE(&buf, buttons)
        write(buf)
    }
    ...
    private func appendUInt32BE(_ data: inout Data, _ value: UInt32) {
    ...
    private func appendInt32BE(_ data: inout Data, _ value: UInt32) {
        appendUInt32BE(&data, value)
    }

    private func appendUInt64BE(_ data: inout Data, _ value: UInt64) {
